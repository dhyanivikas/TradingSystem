...
import pandas as pd
import time
from data_extraction import fetch_ohlcv_data
...
from data_transformation import get_ohlcv_dataframe
from sqlalchemy import text

def harvest_ohlcv_data(engine):
    ...
    for stock_symbol in df['stock_symbol']:
        ...
        query = text("SELECT max(trading_date) FROM stocks_daily_ohlcv_values WHERE stock_symbol = :stock_symbol")
        result = engine.execute(query, {'stock_symbol':stock_symbol})

        max_date = result.scalar()
        print(f"Maximum trading date for {stock_symbol}: {max_date}")
        technical_data = fetch_ohlcv_data(stock_symbol, BBW_DAILY_INTERVAL, BBW_OUTPUT_SIZE, EOD_STOCK_KEY1)
        df = get_ohlcv_dataframe(technical_data)
        ...
        if max_date is None:  # It means the stock was never in the table. persis the dataframe
            ...
            df.to_sql(name='stocks_daily_ohlcv_values', con=engine, if_exists='append', index=False)
        else:
